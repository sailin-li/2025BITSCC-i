...
#define BEGIN_X (<参数自己调>)              // 扫描起始坐标（一个略大于 0 的数）
#define BEGIN_Y (<参数自己调>)              // 近处水平线纵坐标，大一些
#define BEGIN_FAR_Y (<参数自己调>)          // 远处水平线纵坐标，小一些
#define HALF_KERNEL (<参数自己调>)          // 自适应阈值半矩形大小 参考值：7
#define TRI_HALF_KERNEL (<参数自己调>)      // 三角滤波半卷积核大小，参考值：12
#define TP_HALF_KERNEL (<参数自己调>)       // 角点参考点范围一半，参考值：2
#define CLIP_VALUE (<参数自己调>)           // 自适应阈值clip参数，参考：2
#define RESAMPLE_DIS (<参数自己调>)         // 重采样间距，参考：2.8
#define ANG_KERNAL (<参数自己调>)           // 角度获取半卷积核，参考：6
#define LOSS_THRES (<参数自己调>)           // 认为丢线时，左右边点距边距离
#define TRACK_KERNEL (<参数自己调>)         // 寻找当前点法相值前后参考点范围
#define TRACK_DIS   (<参数自己调>)          // 路宽一半
#define LOW_SAMPLE_ACTIVATE (<参数自己调>)  // 某侧可用点数少于此时，激活缺少可用数据点判断方法
// V 对应锐角，L 对应直角，J 对应曲线
#define ANGV_THRES (<参数自己调>)           // 锐角角点最小角度阈值
#define ANGL_MIN_THRES (<参数自己调>)       // 直角角点最小角度阈值
#define ANGL_MIN_THRES_LOW_SAMPLE (<参数自己调>) // 当采样点较少时，适当减少阈值
#define ANGL_MAX_THRES (<参数自己调>)       // 直角角点最大角度阈值
#define CURVE_THRES (<参数自己调>)          // 曲线阈值
#define BW_JUMP_THRES (<参数自己调>)        // 斑马线黑白跳变数量阈值
#define BW_Y (<参数自己调>)                 // 检查斑马线纵坐标
#define BW_Y2 (<参数自己调>)                // 检查斑马线纵坐标2
...
void getTrackMethod(<参数也自己填吧>);                               // 根据需要选择合适的获取目标线的方法
