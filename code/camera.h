...
extern uint8 USE_LOCAL_THRES;                                       // 是否使用局部自适应阈值
...
UPix getLocalThres(UPos x, UPos y);                                 // 计算指定位置的局部自适应阈值
UPix getLocalThresFromWarped(UPos x, UPos y);                       // 通过逆透视坐标计算局部自适应阈值
UPix getMixedThres(UPos x, UPos y);                                 // 获取混合阈值（根据USE_LOCAL_THRES开关）
void setUseLocalThres(uint8 enable);                                // 设置是否使用局部自适应阈值
